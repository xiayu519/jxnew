...
namespace TEngine
{
    internal partial class ResourceExtComponent
    {
        ...
        private void OnLoadAssetSuccess(string assetName, object asset, float duration, object userdata)
        {
            ...
            {
                // 检查资源是否仍然是当前需要的。
                if (IsCurrentLocation(setAssetObject.TargetObject, assetName))
                {
                    ClearLoadingState(setAssetObject.TargetObject);

                    _assetItemPool.Register(AssetItemObject.Create(assetName, assetObject), true);
        ...
        public async UniTaskVoid SetAssetByResources<T>(ISetAssetObject setAssetObject, CancellationToken cancellationToken) where T : UnityEngine.Object
        {
            ...
            var packageName = setAssetObject.PackageName;
            var resourceKey = GetResourceCacheKey(location, packageName);
            ...
            loadingState.Location = resourceKey;
            _loadingStates[target] = loadingState;

            try
            {
                // 等待其他可能正在进行的加载。
                await TryWaitingLoading(resourceKey).AttachExternalCancellation(linkedTokenSource.Token);

                // 再次检查是否被新请求替换。
                if (!IsCurrentLocation(target, resourceKey))
                {
                    return;
                }

                // 检查缓存。
                if (_assetItemPool.CanSpawn(resourceKey))
                {
                    ClearLoadingState(target);

                    var assetObject = (T)_assetItemPool.Spawn(resourceKey).Target;
                    SetAsset(setAssetObject, assetObject);
                }
                else
                {
                    ...
                    // 防止重复加载同一资源。
                    if (!_assetLoadingList.Add(resourceKey))
                    {
                        // 已经在加载中，等待回调处理。
                        return;
                    }

                    T resource = await _resourceModule.LoadAssetAsync<T>(
                        location,
                        linkedTokenSource.Token,
                        packageName);
                    if (resource != null)
                    {
                        _loadAssetCallbacks?.LoadAssetSuccessCallback.Invoke(resourceKey, resource, 0f, setAssetObject);
                        
                    }
                    _assetLoadingList.Remove(resourceKey);
                }
            }
            ...
            catch (Exception ex)
            {
                _assetLoadingList.Remove(resourceKey);
                Log.Error(
                    $"Failed to load asset '{location}' from package " +
                    $"'{packageName}': {ex}");
                ClearLoadingState(target);
            }
        }
