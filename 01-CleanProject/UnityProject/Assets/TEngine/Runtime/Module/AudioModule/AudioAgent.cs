...
namespace TEngine
{
    ...
    public class AudioAgent
    {
        ...
        public static AudioAgent Create(
            string path,
            bool bAsync,
            ...
            string packageName = "")
        {
            ...
            audioAgent.Load(path, bAsync, bInPool, packageName);
            return audioAgent;
        }
        ...
        public void Init(AudioCategory audioCategory, int index = 0)
        {
            ...
            _instanceId = unchecked((int)EntityId.ToULong(_source.GetEntityId()));
#else
            ...
            string packageName = "")
        {
            ...
            if (_audioAgentRuntimeState == AudioAgentRuntimeState.None || _audioAgentRuntimeState == AudioAgentRuntimeState.End)
            {
                ...
                {
                    string poolKey = GetPoolKey(path, _packageName);
                    if (bInPool && _audioModule.AudioClipPool.TryGetValue(poolKey, out var operationHandle))
                    {
                    ...
                    if (bAsync)
                    {
                        ...
                        AssetHandle handle =
                            _resourceModule.LoadAssetAsyncHandle<AudioClip>(
                                path,
                                _packageName);
                    ...
                    {
                        AssetHandle handle =
                            _resourceModule.LoadAssetSyncHandle<AudioClip>(
                                path,
                                _packageName);
        ...
        void OnAssetLoadComplete(AssetHandle handle)
        {
            if (handle != null)
            {
                if (_inPool)
                {
                    _audioModule.AudioClipPool.TryAdd(
                        GetPoolKey(
                            handle.GetAssetInfo().Address,
                            _packageName),
                        handle);
                }
            }
            ...
            if (_pendingLoad != null)
            {
                ...
                string packageName = _pendingLoad.PackageName;
                _pendingLoad = null;
                Load(path, bAsync, bInPool, packageName);
            }
            else if (handle != null)
            {
                if (_audioData != null)
