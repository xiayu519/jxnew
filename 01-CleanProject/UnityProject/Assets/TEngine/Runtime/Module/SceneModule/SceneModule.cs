...
namespace TEngine
{
    internal class SceneModule : Module, ISceneModule
    {
        ...
        public async UniTask<Scene> LoadSceneAsync(string location, LoadSceneMode sceneMode = LoadSceneMode.Single, bool suspendLoad = false, uint priority = 100,
            bool gcCollect = true, Action<float> progressCallBack = null, string packageName = "")
        {
            ...
            else
            {
                ...
                _currentMainScene = LoadSceneHandle(
                    location,
                    sceneMode,
                    suspendLoad,
                    priority,
        ...
        private static SceneHandle LoadSceneHandle(
            string location,
            ...
            string packageName)
        {
            ...
            {
                return YooAssets.LoadSceneAsync(
                    location,
                    sceneMode,
            ...
            if (package == null)
            {
                throw new InvalidOperationException(
                    $"YooAsset package is not initialized: {packageName}");
            }
            return package.LoadSceneAsync(
                location,
                sceneMode,
                LocalPhysicsMode.None,
                suspendLoad,
