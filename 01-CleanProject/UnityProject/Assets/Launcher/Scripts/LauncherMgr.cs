...
namespace Launcher
{
    ...
    public static class LauncherMgr
    {
        ...
        public static void ShowUI<T>(object param = null) where T : UIBase, new()
        {
            ...
            if (!m_uiMapDict.TryGetValue(uiName, out var uiBase))
            {
                ...
                if (obj != null)
                {
                    ...
                    if (uiWindow != null)
                    {
                        uiWindow.transform.SetParent(m_uiRoot.transform, false);
        ...
        public static void RefreshLoadingDescription(object description)
        {
            var ui = GetActiveUI<LoadUpdateUI>();
            if (ui == null)
            {
                ShowUI<LoadUpdateUI>(description);
                return;
            }
            ui.RefreshDescription(description);
        }
