...
namespace GameLogic
{
    ...
    public class UIBase
    {
            ...
            get
            {
                ...
                while (current != null)
                {
                    if (current is UIWindow window)
                        return window.PackageName ?? string.Empty;
        ...
        private static Transform FindChildImp(Transform transform, string path)
        {
            ...
            // Keep the explicit prefab path as the primary contract. Artists
            // may add layout-only wrapper nodes later, so fall back to a
            // descendant lookup only when that original path no longer exists.
            Transform findTrans = transform.Find(path);
            return findTrans != null
                ? findTrans
                : FindBestDescendant(transform, path, null);
        }
        ...
        private static T FindChildComponentImp<T>(Transform transform, string path) where T : Component
        {
            ...
            if (findTrans != null)
            {
                // An existing explicit path remains authoritative. If its
                // component is wrong, return null so the prefab defect is not
                // hidden by an unrelated same-name descendant.
                return findTrans.gameObject.GetComponent<T>();
            }

            Transform fallback = FindBestDescendant(
                transform,
                path,
                typeof(T));
            return fallback != null
                ? fallback.gameObject.GetComponent<T>()
                : null;
        }

        private static Transform FindBestDescendant(
            Transform root,
            string path,
            Type requiredComponentType)
        {
            ...
            Transform best = null;
            int bestMatchedSegments = -1;
            int bestDepth = int.MaxValue;
            FindBestDescendantImp(
                root,
                root,
                pathSegments,
                ...
                1,
                ref best,
                ref bestMatchedSegments,
                ref bestDepth);
            return best;
        }

        private static void FindBestDescendantImp(
            Transform root,
            Transform parent,
            string[] pathSegments,
            Type requiredComponentType,
            ...
            for (int index = 0; index < parent.childCount; index++)
            {
                ...
                if (child.name == leafName &&
                    (requiredComponentType == null ||
                     child.GetComponent(requiredComponentType) != null))
                {
                    int matchedSegments = CountMatchedPathSegments(
                        root,
                        child,
                        pathSegments);
                    if (matchedSegments > bestMatchedSegments ||
                        (matchedSegments == bestMatchedSegments &&
                         depth < bestDepth))
                    {
                        best = child;
                        bestMatchedSegments = matchedSegments;
                        bestDepth = depth;
                    }
                }
        ...
        private static int CountMatchedPathSegments(
            Transform root,
            Transform candidate,
            string[] pathSegments)
        {
            int segmentIndex = pathSegments.Length - 1;
            Transform current = candidate;
            while (current != null && current != root && segmentIndex >= 0)
            {
                if (current.name == pathSegments[segmentIndex])
                {
                    segmentIndex--;
                }
                current = current.parent;
            }
            return pathSegments.Length - segmentIndex - 1;
        }
        ...
        /// <typeparam name="T">UIWidget。</typeparam>
        /// <returns>UIWidget实例。</returns>
        public T CreateWidgetByPath<T>(Transform parentTrans, string assetLocation, bool visible = true, string packageName = null) where T : UIWidget, new()
        {
            GameObject goInst = UIModule.Resource.LoadGameObject(
                assetLocation,
                parent: parentTrans,
                packageName: packageName ?? ResourcePackageName);
        ...
        /// <typeparam name="T">UIWidget。</typeparam>
        /// <returns>UIWidget实例。</returns>
        public async UniTask<T> CreateWidgetByPathAsync<T>(Transform parentTrans, string assetLocation, bool visible = true, string packageName = null) where T : UIWidget, new()
        {
        ...
        /// <typeparam name="T">UIWidget。</typeparam>
        /// <returns>UIWidget实例。</returns>
        public T CreateWidgetByType<T>(Transform parentTrans, bool visible = true, string packageName = null) where T : UIWidget, new()
        {
            return CreateWidgetByPath<T>(
                parentTrans,
                typeof(T).Name,
                visible,
                packageName);
        ...
        /// <typeparam name="T">UIWidget。</typeparam>
        /// <returns>UIWidget实例。</returns>
        public async UniTask<T> CreateWidgetByTypeAsync<T>(Transform parentTrans, bool visible = true, string packageName = null) where T : UIWidget, new()
        {
            return await CreateWidgetByPathAsync<T>(
                parentTrans,
                typeof(T).Name,
                visible,
                packageName);
