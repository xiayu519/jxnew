...
namespace GameLogic
{
    public abstract class UIWidget : UIBase
    {
        ...
        /// <returns></returns>
        public bool CreateByPath(string resPath, UIBase parentUI, Transform parentTrans = null, bool visible = true, string packageName = null)
        {
            GameObject goInst = UIModule.Resource.LoadGameObject(
                resPath,
                parent: parentTrans,
                packageName: packageName ??
                    parentUI?.ResourcePackageName ??
                    string.Empty);
            if (goInst == null)
            {
                return false;
