...
namespace GameLogic
{
    public abstract class UIWindow : UIBase
    {
        ...
        public void Init(string name, int layer, bool fullScreen, string assetName, bool fromResources, int hideTimeToClose, string packageName = "")
        {
            WindowName = name;
            WindowLayer = layer;
        ...
        internal async UniTaskVoid InternalLoad(string location, Action<UIWindow> prepareCallback, bool isAsync, System.Object[] userDatas)
        {
            ...
            if (!FromResources)
            {
                if (isAsync)
                {
                    var uiInstance = await UIModule.Resource.LoadGameObjectAsync(
                        location,
                ...
                {
                    var uiInstance = UIModule.Resource.LoadGameObject(
                        location,
                        parent: UIModule.UIRoot,
                        packageName: PackageName);
                    Handle_Completed(uiInstance);
