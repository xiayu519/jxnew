...
namespace YooAsset.Editor
{
    ...
    public class AssetInfo : IComparable<AssetInfo>
    {
        ...
        public AssetInfo(string assetPath)
        {
            ...
            // Querying the AssetDatabase for a scene's main type causes Unity
            // to open and deserialize the scene. Large scene collections then
            // retain substantial native memory during package collection even
            // though the type is known from the extension.
            AssetType = string.Equals(
                System.IO.Path.GetExtension(AssetPath),
                ".unity",
                StringComparison.OrdinalIgnoreCase)
                ? typeof(UnityEditor.SceneAsset)
                : UnityEditor.AssetDatabase.GetMainAssetTypeAtPath(AssetPath);
